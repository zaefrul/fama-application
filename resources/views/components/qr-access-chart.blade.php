        ...
        <dl class="space-y-2">
            <div>
                <div class="mb-1 flex items-center justify-between text-xs">
                    ...
                    <dd class="font-semibold text-brand">{{ number_format((int) $week) }}</dd>
                </div>
                <div class="h-2 overflow-hidden rounded-full bg-surface-muted">
                    <div class="h-full rounded-full bg-brand" style="width: {{ $weekPct }}%"></div>
                </div>
            </div>
            <div>
                <div class="mb-1 flex items-center justify-between text-xs">
                    <dt class="text-muted">Minggu lepas</dt>
                    <dd class="font-semibold text-ink">{{ number_format((int) $lastWeek) }}</dd>
                </div>
                <div class="h-2 overflow-hidden rounded-full bg-surface-muted">
                    <div class="h-full rounded-full bg-neutral" style="width: {{ $lastPct }}%"></div>
                </div>
            </div>
        </dl>
    </div>
            ...
            @foreach ($daily as $row)
                <div class="flex min-w-0 flex-1 flex-col items-center justify-end gap-1">
                    ...
                    <div class="flex h-24 w-full items-end justify-center">
                        <div class="w-full max-w-8 rounded-t-md bg-brand/80" style="height: {{ max(6, $row['percent']) }}%"></div>
                    </div>
                ...
                @foreach ($top as $row)
                    <li>
                        <div class="mb-1 flex items-start justify-between gap-3 text-xs">
                            ...
                            <p class="shrink-0 font-semibold text-brand">{{ number_format((int) $row['count']) }}</p>
                        </div>
